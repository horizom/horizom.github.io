...
Note that the leading slash separator is _inside_ the placeholder token, not outside.
...
Optional attributes are _sequentially_ optional. This means that, in the above example, you cannot have a "day" without a "month", and you cannot have a "month" without a "year".
...
Note that this is _not_ a content negotiation method. It is only a pro-forma check to see if one of the specified types is present in the `Accept` header with a non-zero `q` value. THe handler, or some other layer, should perform content negotation proper.
...
> N.b.: You can also create a _Rule_ that applies to all routes;
